...
import { RadioButton, Select1, SelectMultiple, Switch, Text, TextArea, TextField } from "../component/export-component";

export function TextFieldForm({ value, label, control, required = false, name, type, placeholder, errors, maxLength, readOnly = false, disabled = false, suffix, prefix, onChange, onBlur, width = '100%' }) {
    return <Controller
        name={name}
        control={control}
        rules={{ required: required ? `Vui lòng ${(placeholder ? placeholder : label ? `Nhập ${label}` : 'gía trị').toLowerCase()}` : null }}
...
export function TextAreaForm({ value, label, control, required = false, name, placeholder, errors, maxLength, readOnly = false, disabled = false, onChange, onBlur, width = '100%' }) {
    return <Controller
        name={name}
        control={control}
        rules={{ required: required ? `Vui lòng ${(placeholder ? placeholder : label ? `Nhập ${label}` : 'gía trị').toLowerCase()}` : null }}
...
export function Select1Form({ value, options, control, label, required = false, name, placeholder, searchPlaceholder, errors, disabled = false, onChange, width = '100%' }) {
    return <Controller
        name={name}
        control={control}
        rules={{ required: required ? `Vui lòng ${(placeholder ? placeholder : label ? `Chọn ${label}` : 'gía trị').toLowerCase()}` : null }}
        render={({ field }) => <div className="col" style={{ gap: '0.8rem', overflow: 'visible', width: width }}>
            {label ? <div className="row" style={{ gap: 4 }}>
                <Text className="label-3">{label}</Text>
                {required ? <Text className="label-4" style={{ color: '#E14337' }}>*</Text> : null}
...
export function SelectMultipleForm({ value, options, control, label, required = false, name, placeholder, errors, disabled = false, onChange, width = '100%' }) {
    return <Controller
        name={name}
        control={control}
        rules={{ required: required ? `Vui lòng ${(placeholder ? placeholder : label ? `Chọn ${label}` : 'gía trị').toLowerCase()}` : null }}
