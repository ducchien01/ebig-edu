...
export default function CourseRatingTab({ rateDetails }) {
    const totalRating = rateDetails ? Object.keys(rateDetails).map(e => e.startsWith('s') ? rateDetails[e] : 0).reduce((a, b) => a + b) : 0
    return <div className="col" style={{ gap: '3.2rem', paddingTop: '2rem' }}>
        ...
        <div className="row" style={{ gap: '4.4rem' }}>
            <div className="col" style={{ gap: '4rem' }}>
                ...
                <div className="col">
                    <div className="heading-3">{totalRating}</div>
            ...
            <div className="col" style={{ flex: 1, gap: '1.4rem' }}>
                {Array.from({ length: 5 }).map((_, i) => {
                    ...
                    return <ProgressBar key={'star-' + i} percent={rateDetails ? (rateDetails[`s${star}`] * 100 / totalRating) : 0} progressBarOnly style={{ width: '100%' }} />
                })}
            </div>
